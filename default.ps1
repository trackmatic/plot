...
Task Publish-Nuget-Package {
	$nuspec = "Plot"
	$spec = [xml](get-content "$base_dir/$nuspec/$nuspec.nuspec")
    ...
    get-childitem "$base_dir/$nuspec/bin/Debug" | ? { $_.Name -like "$nuspec*.dll" -and $_.Name } | % { copy-item $_.FullName "$package_dir/lib/net45" }
